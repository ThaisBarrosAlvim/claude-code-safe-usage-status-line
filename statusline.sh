...
if [ -z "$cli_version" ]; then
    ...
    if [ -n "$cli_version" ]; then
        mkdir -p -m 700 /tmp/claude 2>/dev/null
...
# ===== Usage limits (5h and 7d) =====
# First, try rate_limits data provided directly by Claude Code in the JSON input.
...
if $needs_refresh; then
    ...
    if [ -n "$token" ] && [ "$token" != "null" ]; then
        response=$(curl -s --max-time 10 \
            --config - \
            "https://api.anthropic.com/api/oauth/usage" 2>/dev/null <<EOF
